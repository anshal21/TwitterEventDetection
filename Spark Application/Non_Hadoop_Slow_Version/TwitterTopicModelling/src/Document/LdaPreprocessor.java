...
public class LdaPreprocessor implements Serializable{
    ...
    public LdaPreprocessor(JavaRDD<String> s, SparkSession sparkSession,String outputFile){
        ...
        JavaSparkContext javaSparkContext = new JavaSparkContext(sparkSession.sparkContext());
          ...
          document = s.map((String text) -> {
           ...
           ldaDocuments = document.collect();
            
        
            setRows(ldaDocuments.size());
            debug(ldaDocuments.size());
            setColumns(DocumentProcessor.dictionary.size()+10);
            buildFrequencyMatrix();
            buildInvDict();
            System.out.println(DocumentProcessor.dictionary);
            printMatrix(matrix);
    ...
    public void buildFrequencyMatrix(){
        ...
        while(iterator.hasNext()){
          ...
          //  System.out.println("here "+getRows()+ " "+ getColumns()+" "+ldaDocuments.size()+" "+DocumentProcessor.dictionary.size());
            matrix[rowIndex++] = generateRow(doc);
           /// System.out.println("there "+getRows()+ " "+ getColumns());
        }
    ...
    public int[] generateRow(Document doc){
        ...
        //System.out.println(doc.getWordFrequency());
        Iterator iterator = DocumentProcessor.dictionary.iterator();
        int[] row = new int[getColumns()];
        int colCount = 0;
        
        while(iterator.hasNext()){
            //System.out.println(colCount + " "+ DocumentProcessor.dictionary.size());
            String word = (String)iterator.next();
    ...
    void printMatrix(int[][] matrix){
        ...
        try {
            File file = new File(ProjectConstants.LdaSrcDirectory+outputFile+"input.LDA");
            if(file.exists())
                file.delete();
